...
import { useJobSeekerProfileImage } from "@/hooks/use-jobseeker-profile-image";
...
export function Navbar() {
   ...
   * ============================================================
   *
   * The API is called only when the logged-in
   * user is a JOB_SEEKER.
   *
   * GET:
   * /job-seekers/me/profile-image
   */

  const {
    imageUrl: jobSeekerImageUrl,
    isLoading: profileImageLoading,
  } =
    useJobSeekerProfileImage({
      enabled:
        user?.role ===
   ...
   * IMPORTANT:
   *
   * Do NOT use next/image for the Spring Boot image URL.
   *
   * Spring Boot returns:
   *
   * http://localhost:9000/api/v1/files/uploads/...
   *
   * Next.js Image Optimization tries to proxy this URL and
   * blocks localhost/private IP addresses.
   *
   * Therefore we use normal <img>.
   */
  ...
  const ProfileAvatar = ({
    ...
    return (
      <div
        className={`flex shrink-0 items-center justify-center rounded-full bg-slate-950 text-white ${
          mobile
            ? "h-10 w-10"
            : "h-9 w-9"
        }`}
